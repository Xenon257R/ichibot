...
function generateList(server, type, user) {
    return new Promise((resolve, reject) => {
        if (server === undefined) {
            db.all(`SELECT name, album.track_id, curator, type FROM album`, [], (err, rows) => {
                if (err) reject(err);
                resolve(rows);
            });
        }
...
function generateDefaultList(type) {
    return new Promise((resolve, reject) => {
        db.all(`SELECT name, album.track_id, curator FROM album WHERE (album.curator = ? AND album.type = ?)`, [0, type], (err, rows) => {
            if (err) reject(err);
            resolve(rows);
        })
    })
...
function checkUserImage(server, id) {
    return new Promise((resolve, reject) => {
        db.get(`SELECT custom_image FROM user WHERE (server_id = (?) AND user_id = (?))`, [server, id], (err, row) => {
            if (err) reject(err);
            resolve(row);
        });
    })
...
module.exports = {
    ...
    retrieveList : async function(server, type, user) {
        let list = await generateList(server, type, user);
        if (list.length <= 0) list = await generateDefaultList(type);
        return list;
    },
    checkProfile : async function(server, user) {
        return await checkUserImage(server, user);
    },
