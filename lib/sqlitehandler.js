...
module.exports = {
    ...
    createTag : async function(serverId, userId, tagName) {
        ...
        return new Promise((resolve, reject) => {
            db.run(`INSERT INTO taglist (server_id, user_id, tag_name) VALUES (?, ?, ?)`, [serverId, userId, tagName], (err) => {
                if (err) {
                    switch(err.errno) {
                        case 19:
                            resolve('INUSE');
            ...
            await new Promise((resolve, reject) => {
                db.get(`SELECT tag_name, server_id, user_id, tracks FROM taglist WHERE (tag_name = ? AND server_id = ? AND user_id = ?)`, [tag, serverId, userId], (err, row) => {
                    if (!row || err) {
                        ...
                        db.run(`REPLACE INTO taglist (tag_name, server_id, user_id, tracks) VALUES (?, ?, ?, ?)`, [tag, serverId, userId, JSON.stringify(trackList)], (err) => {
                            if (err) {
                                reject('UNHANDLEDERR');
                                return;
                            };
