...
// Checks if provided url is of type: audio
async function isValidAudioUrl(urlToCheck) {
    ...
    try {
        const response = await fetch(urlToCheck, { method: 'HEAD', signal: AbortSignal.timeout(10000) })
...
module.exports = {
    ...
    addTrack : async function(serverId, userId, name, url, type) {
        ...
        if (/^[^\[\]\(\)"'{}]+$/.test(name)) return(`The track name cannot have any the following characters: \`', (, ), [, ], "\``)
        ...
        let notEdgeCase = ' validated and';
        let edgeCase = '';
        
        // Checks if provided url will work (preliminarily)
        switch(await isValidAudioUrl(url)) {
            case 'BADURL':
            ...
            case false:
                if (url.match(/^(https:\/\/www.dropbox.com\/scl\/fi\/.*)&raw=1$/)) {
                    ...
                    notEdgeCase = '';
                    edgeCase = '\n**CAUTION!**: You provided a *DropBox* URL, a source which **IchiBot** cannot accurately validate. Type `-faq` for further details.';
                    break;
                }
                return (`The provided url <${url}> does not appear to point to an audio source. Addition of entry \`${name}\` has been aborted.\n` +
                    `If you are using services such as Google Drive or Dropbox, double-check to make sure the file's viewing permissions allow IchiBot to access it. Also make sure the URL matches the templates outlined at \`-help mp3\`.`);
            default:
        ...
        return new Promise((resolve, reject) => {
            db.run(`INSERT INTO playlist (server_id, user_id, track_name, track_url, type) VALUES (?, ?, ?, ?, ?)`, [serverId, userId, name, url, type], (err) => {
                ...
                resolve(`\`${name}\`${notEdgeCase} successfully added to the server. ${notice}${edgeCase}`);
            });
        });
    },
