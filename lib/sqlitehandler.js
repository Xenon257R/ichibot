...
module.exports = {
    serverComplete : async function(server) {
        return new Promise((resolve, reject) => {
            db.get(`SELECT command_channel, player_channel FROM server WHERE server_id = (?)`, [server], (err, row) => {
                if (err) reject(false);
                resolve(row && row.command_channel && row.player_channel);
            });
        });
    },
    getServerDetails : async function(server) {
        return new Promise((resolve, reject) => {
            db.get(`SELECT command_channel, player_channel FROM server WHERE server_id = (?)`, [server], (err, row) => {
                if (err) reject(err);
                resolve(row ? row : { command_channel: undefined, player_channel: undefined });
            });
        });
    },
    addServer : async function(server, c, p) {
        return new Promise((resolve, reject) => {
            db.run(`REPLACE INTO server (server_id, command_channel, player_channel) VALUES (?, ?, ?)`, [server, c, p], (err) => {
                if (err) reject (err.message);
                resolve(`Updated server info ${server}.`);
            });
        });
    },
    ...
    retrieveList : async function(server, type, user) {
        return new Promise((resolve, reject) => {
            if (!user) {
                db.all(`SELECT track_name, track_url, user_id, type FROM playlist WHERE (server_id = ?)`, [server], (err, rows) => {
                    if (err) reject(err);
                    resolve(rows);
                });
            }
            db.all(`SELECT track_name, track_url, user_id, type FROM playlist WHERE (user_id = ? AND server_id = ? AND type = ?)`, [user, server, type], (err, rows) => {
                if (err) reject(err);
                resolve(rows);
            });
        }).then((rows) => {
            ...
            if (rows.length <= 0) {
                return new Promise((resolve, reject) => {
                    if (!type) {
                        db.all(`SELECT track_name, track_url, user_id FROM default_album`, [], (err, rows) => {
                            if (err) reject([]);
                            resolve(rows);
                        });
                    }
                    else {
                        db.all(`SELECT track_name, track_url, user_id FROM default_album WHERE (type = ?)`, [type], (err, rows) => {
                            if (err) reject([]);
                            resolve(rows);
                        });
                    }
