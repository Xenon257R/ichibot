...
class serverPlayer {
	constructor(server, voiceChannel, embedChannel, connection, selectedTerm, selectedJukeboxMode) {
		...
		this.buttons = {
			mahjong : [
				...
				new ActionRowBuilder().addComponents(
					new ButtonBuilder().setCustomId('tag1').setLabel('-').setStyle(ButtonStyle.Secondary).setDisabled(true),
					new ButtonBuilder().setCustomId('tag2').setLabel('-').setStyle(ButtonStyle.Secondary).setDisabled(true),
					new ButtonBuilder().setCustomId('tag3').setLabel('-').setStyle(ButtonStyle.Secondary).setDisabled(true),
					new ButtonBuilder().setCustomId('tag4').setLabel('-').setStyle(ButtonStyle.Secondary).setDisabled(true),
					new ButtonBuilder().setCustomId('tag5').setLabel('-').setStyle(ButtonStyle.Secondary).setDisabled(true)
				)
			],
			jukebox : [
				new ActionRowBuilder().addComponents(
	...
	// Modifies Mahjong tag buttons by enabling/disabling them accordingly
	editTagButtons() {
		let buttons = this.buttons.mahjong;
...
module.exports = {
    createVoice : async function(adapters, server, channelId, playerChannel, lastJukeboxMode) {
		...
		instance.collector.on('collect', async i => {
			...
			if (i.customId != 'riichi') instance.activeRiichi = undefined;
			if (i.customId != 'tag1' || i.customId != 'tag2' || i.customId != 'tag3' || i.customId != 'tag4' || i.customId != 'tag5') instance.activeTagList = undefined;
			...
			switch (i.customId) {
				case 'hanchan':
					...
					// Modifies additional button row with tags if created by user
					customButtons = await sqlitehandler.fetchTagList(server_id, i.user.id);
					while (customButtons.length < 5) {
						customButtons.push(null);
					}
					tagButtons = instance.buttons.mahjong;

					for (let j = 0; j < customButtons.length; j++) {
						for (let k = 0; k < 5; k++) {
							if (customButtons[j]) {
								tagButtons[1].components[j].setDisabled(false);
								tagButtons[1].components[j].setLabel(customButtons[j].tag_name);
								tagButtons[1].components[j].setStyle(customButtons[j].mode === 0 ? ButtonStyle.Primary : ButtonStyle.Success);
							}
							else {
								tagButtons[1].components[j].setDisabled(true);
								tagButtons[1].components[j].setLabel('-');
								tagButtons[1].components[j].setStyle(ButtonStyle.Secondary);
							}
						}
					}

					instance.renderEmbed({ embeds: [instance.embed], components : tagButtons });
					...
					break;
				case 'riichi':
					if (!instance.activeRiichi || instance.activeRiichi.user != i.user.id) {
						...
						// Shuffles array
						// CITATION: https://stackoverflow.com/questions/2450954/how-to-randomize-shuffle-a-javascript-array
						let currentIndex = rList.length;
	
						while (currentIndex != 0) {
							let randomIndex = Math.floor(Math.random() * currentIndex);
							currentIndex--;
	
							[rList[currentIndex], rList[randomIndex]] = [rList[randomIndex], rList[currentIndex]];
						}

						instance.activeRiichi = { list: rList, index: 0, user: i.user.id };
					}
					else {
						instance.activeRiichi.index = (instance.activeRiichi.index + 1) % instance.activeRiichi.list.length;
					}
	
					const rSlct = instance.activeRiichi.list[instance.activeRiichi.index];
				...
				case 'tag5':
					...
					if (instance.activeTagList && instance.activeTagList.user === i.user.id && instance.activeTagList.tag === i.customId) {
						tList = instance.activeTagList.list;
						instance.activeTagList.index = (instance.activeTagList.index + 1) % instance.activeTagList.list.length;
						tSlct = instance.activeTagList.index;
					}
					else {
						tList = await sqlitehandler.retrieveList(server_id, 't', i.user.id, instance.buttons.mahjong[1].components[parseInt(i.customId.substring(3)) - 1].data.label);
						if (tList.length <= 0) {
							instance.lock = false;
							i.reply({ content: `There are no tracks tagged with ${instance.buttons.mahjong[1].components[parseInt(i.customId.substring(3)) - 1].data.label} in this server. Apply the tag to your tracks to use this feature.`, ephemeral: true });
							return;
						}
	
						// Shuffles array
						// CITATION: https://stackoverflow.com/questions/2450954/how-to-randomize-shuffle-a-javascript-array
						let currentIndex = tList.length;
						const tMode = await sqlitehandler.checkTagMode(server_id, i.user.id, instance.buttons.mahjong[1].components[parseInt(i.customId.substring(3)) - 1].data.label);
	
						while (currentIndex != 0) {
							let randomIndex = Math.floor(Math.random() * currentIndex);
							currentIndex--;
	
							[tList[currentIndex], tList[randomIndex]] = [tList[randomIndex], tList[currentIndex]];
						}
						instance.activeTagList = { list: tList, index: 0, user: i.user.id, tag: i.customId, mode: tMode.mode };
					}

					instance.embed.description = "Playing customized playlist."
					instance.embed.thumbnail.url = imageUrl[Math.floor(Math.random() * imageUrl.length)];
					instance.updateEmbedTrack(tList[tSlct].track_name, await getUserById(tList[tSlct].user_id, server));
					instance.renderEmbed({ embeds: [instance.embed] });

					instance.player.stop();
					instance.player.removeAllListeners(AudioPlayerStatus.Idle);
					instance.player.play(makeAudioStream(tList[tSlct].track_url));
					instance.player.on(AudioPlayerStatus.Idle, async function() {
						if (instance.activeTagList.mode === 0) {
							console.log('Going to next...');
							tSlct = (tSlct + 1) % tList.length;
							if (tSlct === 0) tList = shuffle(tList);
							instance.updateEmbedTrack(tList[tSlct].track_name, await getUserById(tList[tSlct].user_id, server));
							instance.renderEmbed({ embeds: [instance.embed] });
	
							instance.player.stop();
							instance.player.play(makeAudioStream(tList[tSlct].track_url));
							instance.activeTagList.index = tSlct;
						}
						else {
				...
				case 'game':
					...
					instance.player.stop();
					for (let j = 0; j < 5; j++) {
						instance.buttons.mahjong[1].components[j].setDisabled(true);
						instance.buttons.mahjong[1].components[j].setLabel('-');
						instance.buttons.mahjong[1].components[j].setStyle(ButtonStyle.Secondary);
					}
					instance.resetEmbed();
