...
const { clientId, extender, dev } = require('../config.json');
...
class serverPlayer {
	...
	closeEmbed(timeout) {
		if (timeout) {
			this.embed.thumbnail.url = imageUrl[sadImage[Math.floor(Math.random() * sadImage.length)]];
			this.embed.description = 'Session abandoned nya…'
		}
		else {
			this.embed.thumbnail.url = imageUrl[happyImage[Math.floor(Math.random() * happyImage.length)]];
			this.embed.description = 'Session completed nya!'
		}
		this.embed.fields = [];
...
module.exports = {
    createVoice : async function(adapters, server, channelId, playerChannel) {
		...
		instance.collector.on('collect', async i => {
			...
			switch (i.customId) {
				...
				case 'jukebox':
					...
					instance.renderEmbed({ embeds: [instance.embed], components: instance.buttons.jukebox });
					break;
				case '1':
				case '2':
				case '3':
	...
	forcePlay : async function(serverId, mention, specifiedTrack) {
		...
		switch (instance.mode) {
			...
			case 'mahjong':
				...
				// Easter egg case: either clientId our dev.id have empty riichi playlists
				// NOTE: Will only work in this specific forcing case
				if (rList.length <= 0 && (mention.id === clientId || mention.id === dev.id)) {
					instance.player.play(createAudioResource(dev.track));
					instance.player.removeAllListeners('idle');
					instance.player.on('idle', function() {
						instance.player.play(createAudioResource(dev.track));
					});
					...
					instance.embed.thumbnail.url = dev.profile;
					instance.embed.description = dev.status;
					instance.embed.image.url = dev.image;
					instance.renderEmbed({ embeds: [instance.embed ]});

					return(`\`ERROR (-7) : ${jargon(5, 0)}-${dev.code}-${jargon(5, 0)}\``);
				}
