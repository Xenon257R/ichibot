...
class serverPlayer {
	constructor(server, voiceChannel, embedChannel, connection) {
		...
		for (let i = 0; i < 10; i++) {
			...
			if (this.jukebox.album[j]) {
				...
				if (trackName.length > 30) trackName = trackName.substring(0, 31).trim() + '…';
					...
					if (trackName.length < 30) trackName = trackName + ' ';
					while (trackName.length < 30) trackName = trackName + (this.jukebox.select === j ? '-' : ' ');
				}
...
module.exports = {
    createVoice : async function(server, channel_id, player_channel) {
		...
		activePlayers[server_id] = new serverPlayer(server, channel_id, player_channel, connection);
		const instance = activePlayers[server_id];
		instance.resetEmbed();

		instance.master = await player_channel.send({ embeds: [instance.embed], components: instance.buttons.mahjong });
	...
	forcePlay : async function(server_id, mention, specifiedTrack) {
		...
		switch (instance.mode) {
			...
			case 'mahjong':
				...
				let rList = await sqlitehandler.retrieveList(server_id, 'r', mention.id);
				...
				const customImage = await sqlitehandler.checkProfile(server_id, mention.id);
				if (customImage) instance.embed.thumbnail.url = customImage.custom_image;
