...
// Create number buttons to navigate Jukebox Mode
const jukeboxButton = {
	a : new ButtonBuilder()
		.setCustomId('1')
		.setLabel('01')
	...
	stop : new ButtonBuilder()
		...
		.setLabel('⏹')
		.setStyle(ButtonStyle.Danger),
	gamestyle : new ButtonBuilder()
		.setCustomId('gamestyle')
		.setLabel('🀄')
		.setStyle(ButtonStyle.Success)
}

const mahjongRow = new ActionRowBuilder().addComponents(buttonStyle.hanchan, buttonStyle.riichi, buttonStyle.standby, buttonStyle.dismiss, buttonStyle.jukebox);
const jukeboxRow1 = new ActionRowBuilder().addComponents(jukeboxButton.a, jukeboxButton.b, jukeboxButton.c, jukeboxButton.d, jukeboxButton.e);
const jukeboxRow2 = new ActionRowBuilder().addComponents(jukeboxButton.f, jukeboxButton.g, jukeboxButton.h, jukeboxButton.i, jukeboxButton.j);
const jukeboxRow3 = new ActionRowBuilder().addComponents(jukeboxButton.prevPage, jukeboxButton.stop, jukeboxButton.gamestyle, jukeboxButton.exit, jukeboxButton.nextPage);
...
function generateJukeboxPage(album, page) {
	...
	for (let i = 0 + (page * 10); i < ((page + 1) * 10); i++) {
		if (album[i]) {
			let trackName = album[i].name.replace(new RegExp('_', 'g'), ' ');
			if (trackName.length > 32) trackName = trackName.substring(0, 31).trim() + '…';
			tenTracks = tenTracks + `${(i % 10) < 9 ? '0' : ''}${(i % 10) + 1} | ${trackName}`;
...
module.exports = {
    createVoice : async function(server, channel_id, player_channel) {
        ...
        if (!activePlayers[server_id]) {
			...
			const masterPlayer = await player_channel.send({ embeds: [playerEmbed], components: [mahjongRow] });
				...
				switch (i.customId) {
					case 'hanchan':
						...
						aPlayer.on('idle', function () {
							...
							updateEmbedTrack(playerEmbed, hList[hSlct].name, getUserById(hList[hSlct].curator, server));
					...
					case 'riichi':
						...
						updateEmbedTrack(playerEmbed, rSlct.name, getUserById(rSlct.curator, server), 1, getUserById(i.user.id, server));
						masterPlayer.edit({ embeds: [playerEmbed] });
						break;
					case 'standby':
					case 'gamestyle':
						...
						masterPlayer.edit({ embeds: [playerEmbed], components: [mahjongRow] });
						break;
					case 'dismiss':
					case 'exit':
					...
					case 'jukebox':
						...
						jukeboxPage = 0;
						jukeboxRow3.components[0].setDisabled(true);
						masterPlayer.edit({ embeds: [playerEmbed], components: [jukeboxRow1, jukeboxRow2, jukeboxRow3] });
						break;
					case '1':
					case '2':
					case '3':
					...
					case '10':
						...
						if (jSlct) {
							aPlayer.play(createAudioResource(`./music/${jSlct.type === 0 ? 'hanchan' : 'riichi'}/${jSlct.curator}/${jSlct.name}.mp3`));
							aPlayer.on('idle', function () {
					...
					case 'next':
						...
						if (jukeboxPage <= 0) jukeboxRow3.components[0].setDisabled(true);
						else jukeboxRow3.components[0].setDisabled(false);
						if (jukeboxPage >= (Math.ceil(jukeboxAlbum.length / 10) - 1)) jukeboxRow3.components[4].setDisabled(true);
						else jukeboxRow3.components[4].setDisabled(false);
						playerEmbed.fields = [
							{
								name: 'Tracks:',
