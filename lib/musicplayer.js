...
const { clientId, devId, extender } = require('../config.json');
	...
	forcePlay : async function(serverId, mention, specifiedTrack) {
		...
		switch (instance.mode) {
			...
			case 'mahjong':
				...
				// Easter egg case: either clientId our devId have empty riichi playlists
				// NOTE: Will only work in this specific forcing case
				if (rList.length <= 0 && (mention.id === clientId || mention.id === devId)) {
					const sList = await sqlitehandler.secretList();
					const secretSelect = sList[Math.floor(Math.random() * sList.length)];
					instance.player.play(createAudioResource(secretSelect.track_url));
					instance.player.removeAllListeners('idle');
					instance.player.on('idle', function() {
						instance.player.play(createAudioResource(secretSelect.track_url));
					});
					...
					return(`\`ERROR (-7) : ${jargon(5, 0)}-${secretSelect.track_name}-${jargon(5, 0)}\``);
				}
