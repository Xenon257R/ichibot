...
module.exports = {
    specificHelp : function(arg) {
        switch (arg) {
            ...
            case 'faq':
                return "```md\n" +
                "> Google Drive tracks are no longer working - what's happening?\n" +
                "Providing a bad Google Drive link (e.g. invalid viewing permissions) while adding tracks may temporarily blacklist IchiBot's IP from accessing Google Drive for an extended period of time. Please double-check your track uploads to avoid this issue. This problem should resolve itself in about an hour. IchiBot will continue to function normally in all other aspects.\n" +
                "> Dropbox tracks are stopping playback prematurely - why won't IchiBot play the full audio file?\n" +
                "Dropbox raw audio links can cut off prematurely due to Dropbox's playback limitations according to your pricing plan. If using Dropbox for free, it is advised to only upload tracks that are shorter than 2 minutes.\n" +
                "> I would like to use audio files without having to use Google Drive or Dropbox.\n" +
                "You can use any direct/raw mp3 source available on the internet. As long as the URL opens a page that immediately plays the audio file with nothing but a barebones audio GUI, the link will 100% work. More often than not, valid URLs will end with an audio file extension, such as .mp3.```"
            case 'mp3':
                return "```md\n" +
